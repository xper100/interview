...
> 주로 학습용 데이터가 적을때 사용하며, 전체 data를 batch형태로 나누어 모든 batch가 검증용으로 사용할 수 있게끔 분리하는 형태

- 회귀 / 분류시 알맞은 metric은 무엇일까요?

> 회귀 : MSE -> 실제값과 예측값의 차이를 나타낼 수 있는 metric
> 
> 분류 : Precision(정밀도), Recall(재현율), Accuracy(정확도) -> 분류 결과(discrete)의 신뢰도를 나타낼 수 있는 통계 metric 
...
> 하나의 데이터를 설명하는 정보(feature)의 양이 많을수록 학습이 어려워지는 현상

- dimension reduction기법으로 보통 어떤 것들이 있나요?

> PCA (Principal Component Analysis) : 공분산행렬에 고유값(eigenvalue)로 분해하여 추출한 고유벡터로 새로운 벡터값을 구하는 것 
> 
